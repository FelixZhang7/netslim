...
from netslim import css_thresholding, get_pruning_layers, MaskedBatchNorm
...
def parse_cifar_args(dataset="cifar"):
    default_data_path = {"cifar": "datasets/cifar-10", "ilsvrc12": "/opt/ILSVRC2012"}
...
def ot_post_process_resnet(model):
    scale_weights = list()
    for m in model.modules():
        if isinstance(m, nn.modules.batchnorm._BatchNorm):
            scale_weights.extend([_.abs().item() for _ in list(m.weight)])
    global_prune_th = css_thresholding(scale_weights)

    for m in model.modules():
        if isinstance(m, Bottleneck):
            if m.bn3.weight.abs().max().item() < global_prune_th:
            ...
            if m.bn2.weight.abs().max().item() < global_prune_th:
                m.conv1.apply(deactivate)
                m.bn1.apply(deactivate)
                m.conv2.apply(deactivate)
                m.bn2.apply(deactivate)
...
def ot_post_process_densenet(model):
    scale_weights = list()
    for m in model.modules():
        if isinstance(m, nn.modules.batchnorm._BatchNorm):
            scale_weights.extend([_.abs().item() for _ in list(m.weight)])
    ...
    for m in model.modules():
        if isinstance(m, _DenseLayer):
            if m.norm2.weight.abs().max().item() < global_prune_th:
                m.conv1.apply(deactivate)
