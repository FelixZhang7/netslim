...
def uniform_scaling(weights, prune_ratio, prec_layers, succ_layers):
    ...
    # prune weights with optimal threshold of a specified layer
...
def optimal_thresholding(weights, _, prec_layers, succ_layers, percent=OT_DISCARD_PERCENT):
    """Prune by Optimal Thresholding
    ...
    for norm_layer_name in norm_layer_names:
        ...
        scale_weight = weights[norm_weight_name].abs()
        scale_weight_list = [_.abs().item() for _ in list(scale_weight)]
        prune_th = css_thresholding(scale_weight_list, percent=percent)
        keep_mask = scale_weight > prune_th
        # prune_ratio = 1 - keep_mask.sum().item() / len(scale_weight_list)
    ...
    return weights, None, list()


def global_optimal_thresholding(weights, _, prec_layers, succ_layers, percent=OT_DISCARD_PERCENT):
    """Prune by Optimal Thresholding, globally
    ...
    # find threshold for pruning
    prune_th = css_thresholding(scale_weights, percent=percent)
    ...
    return weights, None, eliminated_layers
...
def prune(model, input_shape, prune_method=optimal_thresholding, prune_ratio=1, channel_select=True):
    """prune a model
